<template>
  <div class="page">
    ...
    <header :class="['toolbar toolbar--top tabbar-toolbar', { 'is-floating': isFloating }]" ref="appBarRef">
      <nav class="tabs-mobile" role="tablist" aria-label="Section Tabs">
        <button
          v-for="s in sections"
          :key="s.id"
          :class="['pill-tab', { active: active === s.id }]"
          role="tab"
          type="button"
...
<style scoped>
.toolbar {
  position: relative; /* default ikut scroll */
  z-index: 1000;
...
/* Tabs (buttons) kecil & cantik */
.tabs-mobile { display: flex; flex-wrap: nowrap; gap: 8px; overflow-x: auto; padding: 4px 4px; }
.tabs-mobile::-webkit-scrollbar { height: 6px; }
.tabs-mobile::-webkit-scrollbar-thumb { background: #e5e7eb; border-radius: 999px; }
...
.pill-tab {
  ...
  background: #f9fafb;
  font-size: .85rem; letter-spacing: .2px; text-transform: none;
  transition: background-color .2s ease, transform .15s ease, border-color .2s ease;
  white-space: nowrap;
}
.pill-tab:hover { background: #f3f4f6; }
.pill-tab.active { background: rgba(37,99,235,0.1); border-color: #2563eb; color: #1f2937; }
...
@media (max-width: 768px) {
  ...
  .pill-tab { min-height: 30px; height: 30px; font-size: .83rem; }
