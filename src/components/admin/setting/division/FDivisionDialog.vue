      ...
      <v-card>
        <v-form v-model="valid" ref="form">
          <v-toolbar class="color-bg-second text-white" density="compact">
            ...
            <v-toolbar-title class="text-subtitle-2">DIVISI/DIV
            ...
            <v-container>
              ...
              <v-row>
                <v-col cols="12" sm="6" md="12">
                  <v-autocomplete
                    v-model="itemModified.fcompanyBean"
                    :items="itemsFCompany"
                    :rules="rulesNotEmpty"
                    item-value="id"
  ...
  methods: {
    showDialog(selectedIndex, item, itemsFCompany) {
      ...
      if (selectedIndex > -1) {
        ...
        (this.itemDefault = new FDivision(0, "", "")),
          (this.itemModified = new FDivision(0, "", "")),
    ...
    save() {
      ...
      if (this.$refs.form.validate()) {
        if (this.formMode === FormMode.EDIT_FORM) {
          FDivisionService.updateFDivision(this.itemModified).then(
            () => {
          ...
          FDivisionService.createFDivision(this.itemModified).then(
            (response) => {
    ...
    initializeEditMode(item) {
      ...
      FDivisionService.getFDivisionById(item.id).then(
        (response) => {
