  ...
  methods: {
    ...
    _clearHighlights() {
      for (const id of this.highlighted) {
        ...
        // Reset style untuk layer yang bisa di-setStyle (Polygon/Line/CircleMarker)
        if (typeof lyr.setStyle === 'function') {
          this._resetStyle(lyr);
        }

        // Reset highlight untuk marker (Point) dengan icon DOM
        if (lyr._icon) {
    ...
    _highlightStyle(layer) {
      ...
      if (geomType === 'LineString' || geomType === 'MultiLineString') {
        // Highlight garis: warna lebih terang, lebih tebal
        layer.setStyle({
          color: '#ff1744',      // merah terang
          weight: 7,             // lebih tebal dari default (4.5)
          opacity: 1,
          dashArray: null,
        });
      } else {
        // Highlight area / titik (polygon, multipolygon, dll)
        layer.setStyle({
          color: '#ff4081',
          weight: 3,
          opacity: 1,
    ...
    jsonToHtmlTable(jsonValue) {
      ...
      for (const meta in myObj) {
        ...
        // If you want to also treat '' as empty, you can add: || String(val).trim() === ''

        // Optionally skip fields where displayVal is empty, or show as blank
        rows += `
      <tr>
