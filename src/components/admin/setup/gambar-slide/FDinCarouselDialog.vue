      ...
      <v-card>
        <v-form v-model="valid" ref="form">
          <v-toolbar class="color-bg-second text-white" density="compact">
            ...
            <v-toolbar-title class="text-subtitle-2">Carousel/Gambar Slide
...
export default {
  ...
  data() {
    return {
      ...
      selectedItemIndex: -1,
      itemDefault: "",
      itemModified: new FDinCarousel(0, "", ""),
  ...
  methods: {
    ...
    saveCreateOnly() {
      if (this.formMode === FormMode.NEW_FORM) {
        FDinCarouselService.createFDinCarousel(this.itemModified).then(
            (response) => {
    ...
    showDialog(selectedIndex, item, itemsFDivision) {
      ...
      if (selectedIndex > -1) {
        ...
        (this.itemDefault = new FDinCarousel(0, "", "")),
          (this.itemModified = new FDinCarousel(0, "", "")),
    ...
    save() {
      ...
      if (this.$refs.form.validate()) {
        if (this.formMode === FormMode.EDIT_FORM) {
          FDinCarouselService.updateFDinCarousel(this.itemModified).then(
            () => {
    ...
    initializeEditMode(item) {
      ...
      FDinCarouselService.getFDinCarouselById(item.id).then(
        (response) => {
