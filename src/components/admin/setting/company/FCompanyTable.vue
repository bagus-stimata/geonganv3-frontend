...
import FCompanyDialog from "./FCompanyDialog";
import FormMode from "@/models/form-mode";
import FCompany from "@/models/f-company";

export default {
  name: "FCompanyTable",
  components: { FCompanyDialog, DeleteDialog },
  data() {
    return {
      snackbar: false,
      snackBarMessage: "",
      ...
      headers: [
        ...
        { title: "Company/DINAS", key: "description",width: "40%" },
        { title: "Aktif", key: "statusActive",width: "16%" },
        { title: "Actions", key: "actions", sortable: false, width: "16%", align: 'center' },
      ],

      formMode: "",
      itemSelectedIndex: -1,
      itemSelected: "",
      fCompanys: [new FCompany(0, "", "")],
      itemsFCompany: [
  ...
  methods: {
    ...
    fetchFCompany() {
      FCompanyService.getAllFCompanyContaining(
        this.currentPage,
        this.pageSize,
        "id",
        "DESC",
    ...
    showDialogEdit(item) {
      this.itemSelectedIndex = this.fCompanysFiltered.indexOf(item);
    ...
    saveDataEdit(item) {
      ...
      Object.assign(this.fCompanys[this.itemSelectedIndex], this.itemSelected);
      this.closeDialog();
    },
    ...
    deleteItemConfirmedSingleSelect(index) {
      ...
      const deletedItem = this.fCompanys[this.itemSelectedIndex];
      FCompanyService.deleteFCompany(deletedItem.id).then(
        () => {
          this.fCompanys.splice(this.itemSelectedIndex, 1);
          this.closeDialog();
        },
        (error) => {
          console.log(error);
    ...
    deleteItemConfirmedMultiSelect(items) {
      if (items.length > -1) {
        ...
        FCompanyService.deleteAllFCompany(itemIds).then(
          (response) => {
            if (response.data) {
              for (let i = 0; i < items.length; i++) {
                if (
                  ...
                  const index = this.fCompanysFiltered.indexOf(items[i]);
                  this.fCompanys.splice(index, 1);
                }
    ...
    closeDialog() {
      ...
      this.$nextTick(() => {
        this.itemSelected = Object.assign({}, new FCompany());
        this.itemSelectedIndex = -1;
      });
    },
