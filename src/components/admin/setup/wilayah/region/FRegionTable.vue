...
import FRegionDialog from "./FRegionDialog";
import FormMode from "@/models/form-mode";
import FRegion from '@/models/f-region'
import FileService from "@/services/apiservices/file-service";

export default {
  components: { FRegionDialog, DeleteConfirmDialog},
  data () {
    return {
      ...
      headers: [
        ...
        { title: 'Nama Kabupaten/Kota', key: 'description', width:"40%" },
        { title: 'Produsen Data/Divisi', key: 'fdivisionBean', width:"30%" },
  ...
  methods: {
    ...
    fetchFRegion() {
      FRegionService.getAllFRegionContaining(this.currentPage, this.pageSize, "id", "DESC", this.search).then(
          response => {
    ...
    showDialogEdit (item) {
      this.itemSelectedIndex = this.fRegionsFiltered.indexOf(item)
    ...
    saveDataEdit(item){
        ...
        try {
          Object.assign(this.fRegions[this.itemSelectedIndex], this.itemSelected)
        }catch (e) {
            this.fetchFRegion()
          e.toString()
        }
    ...
    deleteItemConfirmedSingleSelect(index){
      ...
      const deletedItem = this.fRegions[this.itemSelectedIndex]
      FRegionService.deleteFRegion(deletedItem.id).then(
        () => {
          this.fRegions.splice(this.itemSelectedIndex, 1)
          this.closeDialog()
        },
        error => {
          console.log(error)
    ...
    deleteItemConfirmedMultiSelect(items){
      if (items.length >-1){
        ...
        FRegionService.deleteAllFRegion(itemIds).then(
            response => {
              if (response.data) {
                  for (let i = 0; i < items.length; i++) {
                    if (! response.data.includes(items[i].id)  || response.data.length === 0) {
                      const index = this.fRegionsFiltered.indexOf(items[i])
                      this.fRegions.splice(index, 1)
                    }
    ...
    closeDialog () {
      ...
      this.$nextTick(() => {
        this.itemSelected = Object.assign({}, new FRegion())
        this.itemSelectedIndex = -1
      })
    },
